...
const normalGraph = (props)=>{
    ...
    const convertToRange = (x) => 8 * ((x - margin.left) * 100 / (width - margin.left - margin.right)) / 100 - 4;
    ...
    const selector = svg.append("g")
        .attr('transform', `translate(${(ini* (width-margin.right) / 2 / 4)+width/2-5*(ini-1)},${height/2})`)
    ...
    var dragHandler = d3.drag()
        .on("drag", function (event) {
            selector
                .attr('transform', `translate(${event.x},${(height / 2)})`)
    ...
    const updateData=(x)=>{
        ...
        text.select('#x').text('x='+rounded);
        text.select('#mu').text('μ='+0);
        text.select('#sigma').text('σ='+1);
